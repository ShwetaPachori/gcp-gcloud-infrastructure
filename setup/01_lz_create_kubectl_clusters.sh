...
gcloud container --project $PROJECT_ID clusters create $DEFAULT_CLUSTER_QEA --zone $DEFAULT_ZONE --no-enable-basic-auth --cluster-version $DEFAULT_CLUSTER_VERSION --release-channel "stable" --machine-type "e2-medium" --image-type "COS" --disk-type "pd-standard" --disk-size "100" --metadata disable-legacy-endpoints=true --scopes "https://www.googleapis.com/auth/devstorage.read_only","https://www.googleapis.com/auth/logging.write","https://www.googleapis.com/auth/monitoring","https://www.googleapis.com/auth/servicecontrol","https://www.googleapis.com/auth/service.management.readonly","https://www.googleapis.com/auth/trace.append" --num-nodes "4" --max-nodes 12 --enable-stackdriver-kubernetes --enable-ip-alias --network "projects/"$PROJECT_ID"/global/networks/default" --subnetwork "projects/"$PROJECT_ID"/regions/us-central1/subnetworks/default" --default-max-pods-per-node "110" --enable-autoprovisioning --min-cpu "1" --min-memory "64" --max-cpu "20" --max-memory "256" --no-enable-master-authorized-networks --addons HorizontalPodAutoscaling,HttpLoadBalancing --enable-autoupgrade --enable-autorepair --max-surge-upgrade 1 --max-unavailable-upgrade 0
if [ $? -eq 0 ]
then
  echo "Created Kubernetes '$DEFAULT_CLUSTER_QEA' Cluster"
else
  echo "Failed to create Kubernetes '$DEFAULT_CLUSTER_QEA' Cluster"
fi
